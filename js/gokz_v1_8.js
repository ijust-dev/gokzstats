...
function getMaps(offset) {
  ...
  for (i=offset;i<maplist.length;i++) {
    ...
    $map_div.append("<img onerror=" + "this.src='https://d2u7y93d5eagqt.cloudfront.net/mapImages/thumbs/tn_noimage.jpg'" + " style='width: 150px; height: 84px;' src=" + "https://d2u7y93d5eagqt.cloudfront.net/mapImages/thumbs/tn_" + maplist[i].name + ".jpg>" + "<span style='color: white; padding-left: 15px;'>" + maplist[i].name + "<br>" + "<div>" + getDifficulty(maplist[i].difficulty) + "</div>" + "</span>");
    $("#list_of_maps").append($map_div);
    $map_div.click(function(event){
      if (event.target.parentNode.id.includes("_")) {
  ...
  xhr = $.ajax({
    ...
    success: function(response) {
      ...
      for (m=0;m<5;m++) {
        ...
        $map_div.append("<img onerror=" + "this.src='https://d2u7y93d5eagqt.cloudfront.net/mapImages/thumbs/tn_noimage.jpg'" + " src=" + "https://d2u7y93d5eagqt.cloudfront.net/mapImages/thumbs/tn_" + mostRecentTopTimesFiltered[m].map_name + ".jpg>" + "<span style='color: white;'>" + mostRecentTopTimesFiltered[m].map_name + "<br>" + playerName + "<br>" + "<div>" + timeConvert(mostRecentTopTimesFiltered[m].time) + "</div>" + "<br>" + "<div style='color: white; font-size: 90%; font-style: italic'>" + getTimeDifference(mostRecentTopTimesFiltered[m].created_on) + "</div>" + "</span>");
        $map_div.click(function(event){
          if (event.target.parentNode.id.includes("_")) {
            document.getElementById("searchMap").value = event.target.parentNode.id;
            loadMap(document.getElementById("searchMap").value);
